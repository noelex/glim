...
bool is_matching_cost_factor(const gtsam::NonlinearFactor::shared_ptr& factor) {
...
gtsam::NonlinearFactorGraph GlobalMapping::create_overlapping_factors(const gtsam::Values& values, const gtsam::NonlinearFactorGraph& factors, double min_overlap) const {
  ...
  std::unordered_set<Eigen::Vector3i, gtsam_points::Vector3iHash> existing_matching_pairs;
  for (const auto& factor : factors) {
    // Manual between factors do not suppress a matching-cost factor for the same pair.
    if (!factor || !is_matching_cost_factor(factor) || factor->keys().size() != 2) {
      continue;
    }
    ...
    existing_matching_pairs.emplace(first, second, 0);
  }
  ...
  for (int i = 0; i < submaps.size(); i++) {
    ...
    for (int j = i + 1; j < submaps.size(); j++) {
      ...
      if (existing_matching_pairs.count(Eigen::Vector3i(i, j, 0))) {
        continue;
      }
